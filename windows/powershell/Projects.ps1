...
public enum Project
{
    ...
    Rules = 1024,
    Swaps = 2048,
    All = Automation + Core + Persistence + Web + PopulationAnalytics + ContentAuthoring + Authoring + Common + FdbWeb + Messaging + PlatformCommon + Rules + Swaps
}
"@
...
function global:Get-Projects {
    return @{ 
        ...
        [Project]::Authoring = [PSCustomObject]@{Directory = "~\fdb-rx-authoring"; IncludesNetFrameworkProjects = $true; HasJs = $true; CodeSolution = "~\fdb-rx-authoring\Pharaoh\UI\client"; VsSolution = "~\fdb-rx-authoring\Authoring.sln"; Script = $null; Git = "git@github.com:HearstHealthInternational/fdb-rx-authoring.git" };

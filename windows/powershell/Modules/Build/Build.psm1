...
function Build ([Project] $project = [Project]::All, [Switch] $frontEndOnly, [Switch] $backEndOnly){
    function Build-Project($targetProject){
        ...
        if ($_.Value.VsSolution -ne $null -and -not $frontEndOnly) { 
            ...
            dotnet test $solutionPath --filter FullyQualifiedName!~Smokes --no-build --no-restore --nologo --verbosity m
...
function Run-Server([Project] $project = [Project]::All) {
    function Run($targetProject) {
        if ($_.Value.ServerHost -ne $null) {
            if(!(Get-Process CosmosDB.Emulator -ErrorAction SilentlyContinue)) { CosmosDB.Emulator.exe /NoUi }
